    ...
    <?php if (count($abc) >= 1) : ?>
        ...
        <tr class="file <?php echo (++$i % 2 == 0) ? 'alternate' : '' ?>" id="<?php echo (int)$_POST['fetchU'] ? '' : 's' ?>file<?php echo $file->id ?>">
            ...
            <?php if (!isset($_POST['fetchU'])) {
                ?><td><?php echo stripslashes($file->title) ?></td><?php
            } ?>
            <td style="text-align:center">
                ...
                <?php if (isset($_POST['fetchU'])) {
                    ?><a id="" href="<?php echo admin_url("admin.php?page=sermon-browser/new_sermon.php&amp;getid3={$file->id}"); ?>"><?php _e('Create sermon', 'sermon-browser') ?></a> | <?php
                } ?>
